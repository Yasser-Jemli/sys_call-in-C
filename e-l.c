...
int main(int argc, char* argv[])
{
	...
	int fd[2];
	int ret = pipe(fd);// Appel système de création de tube anonymes
	if(ret == -1) {perror("TUBE"); exit(EXIT_FAILURE);}
	//2
	pid_t val=fork();
	if(val==-1){perror("FORK"); exit(EXIT_FAILURE);}
	//3
	if(val == 0)
	{//FILS
	close(fd[0]);
	...
	int nbOctectLu;
	bzero(BUFFER,10);
	nbOctectLu=read(fd[0],BUFFER, 10);
	while(nbOctect)
	if(nbOctectLu==-1)
		{
			perror("Rd");
